...
### 2026-09-11：ε=0.03 的 PISN 本征事件率

复用 R032、R024–R027 的 1800 万条历史。使用当前 logE IMF、经典
140–260 Msun 命运区间及 Marigo et al. (2003) 的 H+He 燃烧近似延迟，
得到 ηPISN=1.479×10⁻³ Msun⁻¹，延迟 2.43–2.87 Myr。
z=12.5、14.5 的本征率为 (1.294±0.117)×10⁻⁶、(8.361±0.374)×10⁻⁷
yr_src⁻¹ cMpc⁻³，误差仅为 MC 标准误。天空发生率为 0.480、0.232
yr_obs⁻¹ deg⁻² / 单位红移；当前 UV 亮宿主包含 3.76%、0.99% 的事件率。
Marigo 的另一初始质量映射使率提高约 19%、17%，说明命运模型误差需要
单独保留。未重跑 MAH，未加入反馈；未将本征率称为 JWST 检出率。
见 [研究记录](pisn-random-q-study.md)与 [PISN slides](../slides/pisn_random_q_20260911/pisn_random_q.pdf)。

同日补充：PISN slides 第 7–8 页加入 Moriya et al. (2021) HSC 明亮 PISN
数量级上限，以及 JADES/Capotauro 的观测状态。图中只统一源参考系共动率
单位，保留 HSC 的典型 z≈1–3 敏感区域；不把这一上限外推到 z>10，也不
将 Capotauro 的候选解释或理论 CEERS 预期数当成观测率。当前共 14 页。

同日更正：前述 Capotauro 状态核查遗漏 Liu et al. (2026-08-07,
arXiv:2608.07461)。其新 NIRCam 天体测量发现 132±20 mas 的位置移动，
以 >6σ 排除河外源，支持 Y 型褐矮星。已更新第 8 页及观测资料条目，
不再把 Capotauro 列为当前待确认的高红移 PISN 候选。

### 2026-09-11：登记 JWST/PISN 观测比较 EXP

用户确认将“按 JWST 实际观测条件预测 PISN 检出数并与样本比较”标记为
EXP。沿用当前 εb=0.03 模型，已有本征率作为基础；光变谱、滤镜、历元、
差分噪声、选择效率和有效可见时间的前向计算待开展。
详见 [EXP 登记](pisn-random-q-study.md#expjwstpisn-检出预测与观测比较)。
本次仅登记实验，未启动新运行。
