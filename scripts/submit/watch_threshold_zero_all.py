...
EVENT_ID = "auroralf-cp6-threshold-zero-11690655-20260918"
JOB = "11690655"
...
def query():
    command = "sacct -X -n -P -j 11690655 --format=JobID,Cluster,Submit,User,State%40,ExitCode"
    ...
    for line in result.stdout.splitlines():
        ...
        if (
            row["cluster"] != "hpc5"
            or row["user"] != "xuelei"
            or row["submitted"] != "2026-09-18T10:44:09"
        ):
            raise ValueError("Remote scheduler identity mismatch")
        match = re.fullmatch(r"11690655_\[([0-9,%-]+)\]", row["job"])
        if match:
            for part in match[1].split("%", 1)[0].split(","):
                ends = list(map(int, part.split("-")))
                for i in range(ends[0], ends[-1] + 1):
                    rows.append(dict(row, job=f"{JOB}_{i}"))
        else:
            rows.append(row)
    if {r["job"] for r in rows} != {f"{JOB}_{i}" for i in range(19)} or len(rows) != 19:
        raise ValueError("Expected all nineteen array elements")
    return rows
    ...
    with (OUT / "event.lock").open("a") as lock:
        ...
        while state["phase"] != "delivered":
            try:
                ...
                if (failed or len(terminal) == 19) and state["phase"] == "watching":
                    state["phase"] = "waiting"
                write(state)
                if args.check_only:
