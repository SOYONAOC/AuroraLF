...
array over SSH; contacts Codex only on completion/failure and waits for idle.
...
EVENT_ID = "auroralf-cp6-threshold-zero-11690033-20260918"
JOB = "11690033"
...
def query():
    command = "sacct -X -n -P -j 11690033 --format=JobID,Cluster,Submit,User,State%40,ExitCode"
    ...
    for line in result.stdout.splitlines():
        ...
        if (
            row["cluster"] != "hpc5"
            or row["user"] != "xuelei"
            or row["submitted"] != "2026-09-18T09:58:54"
        ):
            raise ValueError("Remote scheduler identity mismatch")
        rows.append(row)
    if {r["job"] for r in rows} != {f"{JOB}_{i}" for i in range(10)} or len(rows) != 10:
        raise ValueError("Expected all ten array elements")
    return rows
...
def deliver(state, thread):
    ...
    try:
        ...
        payload = dict(
            event_id=EVENT_ID,
            source="Authorized remote cp6 campaign; all array identities checked",
            accounting=state["accounting"],
            followup="Continue the user's mu=0 UVLF rerun: inspect failures or retrieve completed products, run scripts/analysis/finish_threshold_zero_uvlf.py, visually review and replace slides 8/9, compile and sync Zotero. See outputs/threshold_zero_campaign_20260918/FOLLOWUP.md. Preserve unrelated slide edits. Do not notify unchanged running states.",
        )
        try:
            result = client.call(
                "turn/start",
    ...
    with (OUT / "event.lock").open("a") as lock:
        ...
        while state["phase"] != "delivered":
            try:
                ...
                if (failed or len(terminal) == 10) and state["phase"] == "watching":
                    state["phase"] = "waiting"
                write(state)
                if args.check_only:
