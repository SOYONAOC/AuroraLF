...
TNG_MAH_CACHE_SCHEMA_VERSION = "auroralf_tng_mah_cache_v2"
...
_TNG_MAH_CACHE: dict[tuple[FileVersion, float], dict[str, Any]] = {}
...
def _read_required_bool_dataset(handle: h5py.File, name: str) -> np.ndarray:
    ...
    if values.dtype.kind != "b":
        raise ValueError(f"TNG MAH cache {name} must use a native bool dtype")
    return values


def _read_required_text_attr(handle: h5py.File, name: str) -> str:
    if name not in handle.attrs:
        raise KeyError(f"TNG MAH cache is missing required provenance attribute '{name}'")
    raw_value = handle.attrs[name]
    if isinstance(raw_value, bytes):
        try:
            value = raw_value.decode("utf-8")
        except UnicodeDecodeError as exc:
            raise ValueError(f"TNG MAH cache provenance attribute '{name}' must be UTF-8 text") from exc
    elif isinstance(raw_value, str):
        value = raw_value
    else:
        raise ValueError(f"TNG MAH cache provenance attribute '{name}' must be text")
    value = value.strip()
    if not value or value.lower() in _PROVENANCE_PLACEHOLDERS:
        raise ValueError(
            f"TNG MAH cache provenance attribute '{name}' must contain a real value; got {value!r}"
        )
    return value
...
def _read_required_nonnegative_int_attr(handle: h5py.File, name: str) -> int:
    ...
    if isinstance(raw_value, (bool, np.bool_)) or not isinstance(raw_value, (int, np.integer)):
        raise ValueError(f"TNG MAH cache provenance attribute '{name}' must be an integer")
    value = int(raw_value)
    if value < 0:
        raise ValueError(f"TNG MAH cache provenance attribute '{name}' must be non-negative")
    return value
...
def _read_required_identifier_dataset(handle: h5py.File, name: str) -> np.ndarray:
    ...
    if values.dtype.kind not in "iu":
        raise ValueError(f"TNG MAH cache {name} must contain integer identifiers")
    if values.dtype.kind == "u" and np.any(values > np.iinfo(np.int64).max):
        raise ValueError(f"TNG MAH cache {name} values must fit in signed 64-bit integers")
    identifiers = np.asarray(values, dtype=np.int64)
    if np.any(identifiers < 0):
        raise ValueError(f"TNG MAH cache {name} must contain real non-negative identifiers")
    return identifiers
...
def _read_required_text_dataset(handle: h5py.File, name: str) -> np.ndarray:
    ...
    if raw_values.ndim != 1 or raw_values.size == 0:
        raise ValueError(f"TNG MAH cache {name} must be a non-empty 1D text array")
    decoded: list[str] = []
    for raw_value in raw_values:
        if isinstance(raw_value, bytes):
            try:
                ...
                raise ValueError(f"TNG MAH cache {name} must contain UTF-8 text") from exc
        elif isinstance(raw_value, str):
            value = raw_value
        else:
            raise ValueError(f"TNG MAH cache {name} must contain text values")
        value = value.strip()
        if not value or value.lower() in _PROVENANCE_PLACEHOLDERS:
            raise ValueError(f"TNG MAH cache {name} must contain real source values; got {value!r}")
...
def _validate_source_file_checksums(
    ...
    if source_file_sha256.shape != source_file_identifier.shape:
        raise ValueError(
            "TNG MAH cache source_file_sha256 must match source_file_identifier shape"
        )
    ...
    for identifier, checksum in zip(
        ...
        if len(checksum) != 64 or any(character not in _SHA256_HEXDIGITS for character in checksum):
            raise ValueError(
                "TNG MAH cache source_file_sha256 must contain 64-character hexadecimal SHA-256 values"
            )
        previous = identifier_to_checksum.setdefault(identifier, checksum.lower())
        if previous != checksum.lower():
            raise ValueError(
                "TNG MAH cache source_file_identifier cannot map to multiple SHA-256 values"
            )


def _read_required_cosmology_attr(handle: h5py.File, name: str) -> float:
    if name not in handle.attrs:
        raise KeyError(f"TNG MAH cache is missing required cosmology attribute '{name}'")
    try:
        value = float(handle.attrs[name])
    except (TypeError, ValueError) as exc:
        raise ValueError(
            f"TNG MAH cache cosmology attribute '{name}' must be numeric"
        ) from exc
    if not np.isfinite(value):
        raise ValueError(f"TNG MAH cache cosmology attribute '{name}' must be finite")
    return value
...
def _validate_cache_cosmology(cache: dict[str, Any], cosmology: Cosmology) -> None:
    ...
    for field, expected_value in expected.items():
        ...
        if not np.isclose(cache_value, float(expected_value), rtol=0.0, atol=1.0e-12):
            raise ValueError(
                f"TNG MAH cache cosmology {field}={cache_value:.16g} does not match "
...
def _read_cache_z_final(handle: h5py.File, z_grid: np.ndarray) -> float:
    if "z_final" in handle.attrs:
        try:
            ...
            raise ValueError("TNG MAH cache attribute 'z_final' must be numeric") from exc
...
def _require_current_file_version(version: FileVersion) -> None:
    try:
        ...
        raise RuntimeError(f"TNG MAH cache changed during preload: {version.path}") from exc
    if current != version:
        raise RuntimeError(f"TNG MAH cache changed during preload: {version.path}")
...
def _read_tng_cache_file(
    ...
    with resolved.open("rb") as raw_handle:
        ...
        if opened_version != version:
            raise RuntimeError(f"TNG MAH cache changed during preload: {resolved}")
    ...
    if time_unit != "Gyr":
        raise ValueError(f"TNG MAH cache time_unit must be 'Gyr'; got {time_unit!r}")
    if redshift_unit != "dimensionless":
        raise ValueError(
            "TNG MAH cache redshift_unit must be 'dimensionless'; "
            f"got {redshift_unit!r}"
        )
    if mass_ratio_unit != "dimensionless":
        raise ValueError(
            "TNG MAH cache mass_ratio_unit must be 'dimensionless'; "
            f"got {mass_ratio_unit!r}"
        )
    if hubble <= 0.0:
        raise ValueError("TNG MAH cache cosmology attribute 'hubble' must be positive")
    if omega_m <= 0.0:
        raise ValueError("TNG MAH cache cosmology attribute 'omega_m' must be positive")
    if omega_b <= 0.0 or omega_b > omega_m:
        raise ValueError("TNG MAH cache cosmology attribute 'omega_b' must lie in (0, omega_m]")
    ...
    if source_snapshot.ndim != 1 or source_snapshot.size != mass_ratio.shape[0]:
        raise ValueError("TNG MAH cache source_snapshot must be 1D and match mass_ratio rows")
    if np.any(source_snapshot != snapshot):
        raise ValueError("TNG MAH cache source_snapshot values must match snapshot attribute")
    source_identity = np.column_stack((source_snapshot, source_subhalo_id))
    if np.unique(source_identity, axis=0).shape[0] != source_identity.shape[0]:
        raise ValueError("TNG MAH cache source halo identities must be unique")
    ...
    if not np.all(resolved_mask[:, -1]):
        raise ValueError("TNG MAH cache mass_ratio final column must be resolved for every halo")
    if not np.all(np.isfinite(mass_ratio[:, -1])) or not np.allclose(
        ...
        raise ValueError("TNG MAH cache mass_ratio final resolved column must equal 1")
...
def _freeze_tng_cache(cache: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _clear_tng_mah_cache_for_tests() -> None:
    _TNG_MAH_CACHE.clear()


def preload_tng_mah_cache(
    cache_path: str | Path,
    *,
    z_final: float,
    cosmology: Cosmology,
) -> Path:
    return _preload_tng_mah_cache_version(
        cache_path,
        z_final=z_final,
        cosmology=cosmology,
    ).path
...
def _preload_tng_mah_cache_version(
    cache_path: str | Path,
    *,
    z_final: float,
    cosmology: Cosmology,
    ...
    cached = _TNG_MAH_CACHE.get(key)
    if cached is not None:
        _require_current_file_version(version)
        _validate_cache_cosmology(cached, cosmology)
        return version
    loaded_path, loaded = _read_tng_cache_file(version, z_final=float(z_final))
    if loaded_path != version.path:
        raise RuntimeError("resolved TNG cache path changed during preload")
    _validate_cache_cosmology(loaded, cosmology)
    frozen = _freeze_tng_cache(loaded)
    _TNG_MAH_CACHE[key] = frozen
    return version
...
def _load_tng_cache(
    ...
    *,
    ...
    version = _preload_tng_mah_cache_version(
        cache_path,
        z_final=float(z_final),
        cosmology=cosmology,
    )
    return version.path, _TNG_MAH_CACHE[(version, float(z_final))]
...
def generate_tng_halo_histories(
    ...
    *,
    ...
    resolved_cache_path, cache = _load_tng_cache(
        cache_path,
        z_final=float(z_final),
        cosmology=cosmology,
    )
